...
# 1. 基础配置
# ==========================================
st.set_page_config(page_title="智能调拨系统 V34.0 (提货优先版)", layout="wide", page_icon="🦁")
...
st.title("🦁 智能库存分配 V34.0 (提货计划优先 + 双向统筹)")
...
def load_and_find_header(file):
    ...
    try:
        ...
        for i, row in df.head(50).iterrows():
            found = False
            for v in row.values:
                val_clean = str(v).upper().replace(' ', '').replace('\n', '').replace('\r', '')
                if any(k in val_clean for k in ["SKU", "FNSKU", "编码", "未入库", "贴标要求", "数量"]):
                    header_idx = i
                    found = True
                    break
            if found: break
...
class InventoryManager:
    ...
    def _init_inventory(self, df):
        ...
        if not (c_sku and c_wh and c_qty): 
            self.cleaning_logs.append({"类型": "错误", "SKU": "-", "原因": "【库存表】列名识别失败"})
            return
        ...
        for idx, row in df.iterrows():
            ...
            if any(k in normalize_str(w_name_raw) for k in ["沃尔玛", "WALMART", "TEMU"]): continue
            if not sku: continue
            
            fnsku = str(row.get(c_fnsku, '')).strip().upper() if pd.notna(row.get(c_fnsku, '')) else ""
            ...
            if qty > 0:
                ...
                if fnsku not in self.stock[sku]: self.stock[sku][fnsku] = {'深仓':[], '外协':[], '云仓':[], '采购订单':[], '提货计划':[]}
    ...
    def _init_po(self, df):
        ...
        for idx, row in df.iterrows():
            sku = str(row.get(c_sku, '')).strip().upper() 
            if c_req and any(b in str(row.get(c_req, '')) for b in block_list): continue
    ...
    def _init_plan(self, df):
        if df is None or df.empty: return
        c_sku = self._match_col(df, ['SKU', '编码', '代码'])
        c_fnsku = self._match_col(df, ['FNSKU', '条码', '贴标要求'])
    ...
    def get_supply_by_sku_fnsku(self, sku, fnsku):
        ...
        if sku in self.stock and fnsku in self.stock[sku]:
            for w in self.stock[sku][fnsku]: total += sum(i['qty'] for i in self.stock[sku][fnsku][w])
        if sku in self.po and fnsku in self.po[sku]: total += sum(i['qty'] for i in self.po[sku][fnsku])
        if sku in self.plan and fnsku in self.plan[sku]: total += sum(i['qty'] for i in self.plan[sku][fnsku])
        return total
    ...
    def get_total_supply(self, sku):
        ...
        if sku in self.stock:
            for f in self.stock[sku]:
                for w in self.stock[sku][f]: total += sum(i['qty'] for i in self.stock[sku][f][w])
        if sku in self.po:
            for f in self.po[sku]: total += sum(i['qty'] for i in self.po[sku][f])
        if sku in self.plan:
            for f in self.plan[sku]: total += sum(i['qty'] for i in self.plan[sku][f])
        return total
    ...
    def get_snapshot(self, sku):
        ...
        if sku in self.stock:
            for f in self.stock[sku]:
                for w_type in ['深仓', '外协', '云仓']: res[w_type] += sum(item['qty'] for item in self.stock[sku][f].get(w_type, []))
        if sku in self.po:
            for f in self.po[sku]: res['采购订单'] += sum(item['qty'] for item in self.po[sku][f])
        if sku in self.plan:
            for f in self.plan[sku]: res['提货计划'] += sum(item['qty'] for item in self.plan[sku][f])
        return res
    ...
    def execute_deduction(self, sku, target_fnsku, qty_needed, strategy_chain, mode='strict_only'):
        ...
        for src_type, src_name in strategy_chain:
            ...
            # --- 2. PO 或 PLAN 扣减 ---
            elif src_type in ['po', 'plan']:
                target_dict = self.po if src_type == 'po' else self.plan
                if sku in target_dict:
                    if mode in ['po_any', 'plan_any', 'strict_only']:
                        targets = [target_fnsku] if mode == 'strict_only' else list(target_dict[sku].keys())
                        for f in targets:
                            if f not in target_dict[sku]: continue
                            if qty_remain <= 0: break
                            for item in target_dict[sku][f]:
                        ...
                        for other_f in target_dict[sku]:
                            if other_f == target_fnsku: continue
                            if qty_remain <= 0: break
                            for item in target_dict[sku][other_f]:
                                if item['raw_name'] != src_name: continue
                                if qty_remain <= 0: break
                                if item['qty'] <= 0: continue
                                take = min(item['qty'], qty_remain)
...
def run_allocation(df_input, inv_mgr, mapping):
    ...
    for sku, req_qty in demand_summary.items():
        ...
        if gap > 0:
            order_list.append({
                ...
                "现有全盘供应(计+库+PO)": to_int(total_supply), "建议真实下单数量": to_int(gap)
            })
    ...
    # 策略定义 (分离 Plan, Stock, PO)
    strat_plan = [('plan', '提货计划')]
    strat_po = [('po', '采购订单')]
    ...
    # --- Tier 1: 非 US (顺序：Plan > Stock > PO) ---
    if tiers[1]:
        for t in tiers[1]: # R1: Plan 精准
            rem = t['qty'] - t['filled']
            if rem > 0:
                r, u, p, l = inv_mgr.execute_deduction(t['sku'], t['fnsku'], rem, strat_plan, 'strict_only')
                update_task(t, r, u, p, [f"[R1提货精准]:{x}" for x in l])
        for t in tiers[1]: # R2: Plan 加工
            rem = t['qty'] - t['filled']
            if rem > 0:
                r, u, p, l = inv_mgr.execute_deduction(t['sku'], t['fnsku'], rem, strat_plan, 'process_only')
                update_task(t, r, u, p, [f"[R2提货加工]:{x}" for x in l])
        for t in tiers[1]: # R3: Stock 现货精准
            rem = t['qty'] - t['filled']
            if rem > 0:
                r, u, p, l = inv_mgr.execute_deduction(t['sku'], t['fnsku'], rem, strat_stock_non_us, 'strict_only')
                update_task(t, r, u, p, [f"[R3现货精准]:{x}" for x in l])
        for t in tiers[1]: # R4: Stock 现货加工
            rem = t['qty'] - t['filled']
            if rem > 0:
                r, u, p, l = inv_mgr.execute_deduction(t['sku'], t['fnsku'], rem, strat_stock_non_us, 'process_only')
                update_task(t, r, u, p, [f"[R4现货加工]:{x}" for x in l])
        for t in tiers[1]: # R5: PO 盲配
            rem = t['qty'] - t['filled']
            if rem > 0:
                r, u, p, l = inv_mgr.execute_deduction(t['sku'], t['fnsku'], rem, strat_po, 'po_any')
                update_task(t, r, u, p, [f"[R5采购盲配]:{x}" for x in l])
    ...
    # --- Tier 2: US (原则：全部精准优先，再找加工) ---
    if tiers[2]:
        # 全场精准匹配轮次
        for t in tiers[2]: # R1: Plan 精准
            rem = t['qty'] - t['filled']
            if rem > 0:
                r, u, p, l = inv_mgr.execute_deduction(t['sku'], t['fnsku'], rem, strat_plan, 'strict_only')
                update_task(t, r, u, p, [f"[R1提货精准]:{x}" for x in l])
        for t in tiers[2]: # R2: Stock 精准
            rem = t['qty'] - t['filled']
            if rem > 0:
                r, u, p, l = inv_mgr.execute_deduction(t['sku'], t['fnsku'], rem, strat_stock_us, 'strict_only')
                update_task(t, r, u, p, [f"[R2现货精准]:{x}" for x in l])
        for t in tiers[2]: # R3: PO 精准
            rem = t['qty'] - t['filled']
            if rem > 0:
                r, u, p, l = inv_mgr.execute_deduction(t['sku'], t['fnsku'], rem, strat_po, 'strict_only')
                update_task(t, r, u, p, [f"[R3采购精准]:{x}" for x in l])
                
        # 全场加工兜底轮次
        for t in tiers[2]: # R4: Plan 加工
            rem = t['qty'] - t['filled']
            if rem > 0:
                r, u, p, l = inv_mgr.execute_deduction(t['sku'], t['fnsku'], rem, strat_plan, 'process_only')
                update_task(t, r, u, p, [f"[R4提货加工]:{x}" for x in l])
        for t in tiers[2]: # R5: Stock 加工
            rem = t['qty'] - t['filled']
            if rem > 0:
                r, u, p, l = inv_mgr.execute_deduction(t['sku'], t['fnsku'], rem, strat_stock_us, 'process_only')
                update_task(t, r, u, p, [f"[R5现货加工]:{x}" for x in l])
        for t in tiers[2]: # R6: PO 加工
            rem = t['qty'] - t['filled']
            if rem > 0:
                r, u, p, l = inv_mgr.execute_deduction(t['sku'], t['fnsku'], rem, strat_po, 'process_only')
                update_task(t, r, u, p, [f"[R6采购加工]:{x}" for x in l])
    ...
    output_rows = []
    # 提货计划排在最前面显示
    display_order = ['提货计划', '深仓', '外协', '云仓', '采购订单']
    display_map = {'提货计划':'提货计划', '深仓':'深仓库存', '外协':'外协仓库存', '云仓':'云仓库存', '采购订单':'采购订单'}
    ...
    for idx, row in df_input.iterrows():
        ...
        if t:
            ...
            out_row.update({
                ...
                "剩_计划": to_int(snap['提货计划']), "剩_深仓": to_int(snap['深仓']), "剩_外协": to_int(snap['外协']),
...
with col_main:
    st.subheader("1. 需求填报 (V34.0 提货优先版)")
...
with col_side:
    ...
    f_po = st.file_uploader("B. 采购追踪表 (在途PO)", type=['xlsx', 'xls', 'csv'])
    f_plan = st.file_uploader("C. 提货计划表 (优先在途)", type=['xlsx', 'xls', 'csv'])
    
    if st.button("🚀 执行提货优先分配", type="primary", use_container_width=True):
        if f_inv and f_po and not edited_df.empty:
            with st.spinner("执行供需预判及提货优先分配..."):
                ...
                if err1: st.error(err1)
                    ...
                    tab1, tab2, tab3 = st.tabs(["📋 分配明细", "🔍 逻辑日志", "✅ 数据诊断雷达"])
                    ...
                    st.download_button("📥 下载完整报告.xlsx", buf.getvalue(), "V34_Result.xlsx")
        else:
