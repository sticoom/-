...
# 1. 基础配置
# ==========================================
st.set_page_config(page_title="智能调拨系统 V8.0 (经典整单优先版)", layout="wide", page_icon="🦁")
...
class InventoryManager:
    ...
    def execute_deduction(self, sku, target_fnsku, qty_needed, strategy_chain):
        ...
        return qty_needed - qty_remain, used_sources, process_details
...
def run_allocation(df_input, inv_mgr, df_plan, mapping):
    ...
    for idx, row in df_input.iterrows():
        ...
        # V8 经典优先级: 
        # 新增(Non-US) > 新增(US) > 当周(Non-US) > 当周(US)
        priority = 0
    ...
    for t in tasks:
        ...
        if not t['is_us']:
            # Non-US: 瀑布流 (深 > 外 > 云 > PO) - V8.1 逻辑
            strat = [('stock', '深仓'), ('stock', '外协'), ('stock', '云仓'), ('po', '采购订单')]
            filled, srcs, proc = inv_mgr.execute_deduction(sku, fnsku, qty, strat)
        else:
            # US: 整单优先
            candidates = [('stock', '外协'), ('stock', '云仓'), ('po', '采购订单'), ('stock', '深仓')]
            whole_match = inv_mgr.find_whole_match(sku, fnsku, qty, candidates)
            
            if whole_match:
                filled, srcs, proc = inv_mgr.execute_deduction(sku, fnsku, qty, whole_match)
            else:
                # 兜底: 外 > 云 > PO > 深
                strat = [('stock', '外协'), ('stock', '云仓'), ('po', '采购订单'), ('stock', '深仓')]
                filled, srcs, proc = inv_mgr.execute_deduction(sku, fnsku, qty, strat)

        results[rid] = {'filled': filled, 'srcs': srcs, 'proc': proc}
...
with col_side:
    ...
    if st.button("🚀 开始计算", type="primary", use_container_width=True):
        if f_inv and f_po and not edited_df.empty:
            ...
            st.download_button("📥 下载结果.xlsx", buf.getvalue(), "V8_Result.xlsx")
